...
const ProfilePage = () => {
  ...
  const { state } = useLocation(); // Get the category from navigation state
  const [name, setName] = useState(null);
  const [votes, setVotes] = useState(0);
  const [profileImage, setProfileImage] = useState(null);
  const [category, setCategory] = useState(state?.category || ""); // Use category from state
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Don’t redirect on 401, just log the error and proceed
      }
    };

    if (candidateId !== "0") { // Only fetch if candidateId is valid
      fetchData();
    }
  }, [candidateId]);
