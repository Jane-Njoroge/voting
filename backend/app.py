...
app.config["JWT_SECRET_KEY"] = os.getenv("JWT_SECRET_KEY", "32")
print("JWT_SECRET_KEY loaded as:", app.config["JWT_SECRET_KEY"])
...
def get_candidates():
    ...
    candidate_list = [{'id': candidate.id, 'full_name': candidate.full_name, 'email': candidate.email} for candidate in candidates]
...
def get_name_profile_image(candidate_id):
    ...
    return jsonify({'full_name': candidate.full_name, 'profileImage': profile_image}), 200
...
def verify_and_send_otp():
    ...
    try:
        otp_verification = client.verify.v2.services(TWILIO_VERIFY_SERVICE_SID).verifications.create(to=phone_number, channel='sms')
        return jsonify({
...
def verify_otp():
    ...
    try:
        verification_check = client.verify.v2.services(TWILIO_VERIFY_SERVICE_SID).verification_checks.create(to=phone_number, code=otp)
...
def assign_category():
    data = request.get_json()
    candidate_id = data.get('candidate_id')  # Get from request body prefered to get from the body than using session
    category = data.get('category')

    print(f"Received data from frontend: {data}")
    print(f"Extracted candidate_id: {candidate_id}, category: {category}")

    if not candidate_id:
        print("Candidate ID is missing in the request!")
...
def get_candidates_by_category(category):
    try:
        ...
        candidates_list = [{'id': candidate.id, 'name': candidate.full_name} for candidate in candidates]
...
def get_candidates_with_categories():
    try:
        candidates = db.session.query(Candidate.id, Candidate.full_name, Candidate.email, Verification.category).join(Verification, Candidate.id == Verification.candidate_id).all()
        candidate_list = [{"id": c.id, "full_name": c.full_name, "email": c.email, "category": c.category} for c in candidates]
        return jsonify({"candidates": candidate_list}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_all_candidates_with_categories():
    try:
        candidates = db.session.query(Candidate.id, Candidate.full_name, Candidate.email, Verification.category).outerjoin(Verification, Candidate.id == Verification.candidate_id).all()
        candidate_list = [{"id": c.id, "full_name": c.full_name, "email": c.email, "category": c.category if c.category else "No Category"} for c in candidates]
        return jsonify({"candidates": candidate_list}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def candidates_without_category():
    candidates = db.session.query(Candidate).filter(~Candidate.id.in_(db.session.query(Verification.candidate_id).filter(Verification.category.isnot(None)))).all()
    if not candidates:
        return jsonify({'message': 'All candidates have categories'}), 200
    result = [{'id': c.id, 'name': c.full_name} for c in candidates]
